...
public class UpdateManager {
    ...
    private Handler mHandler = new Handler(){
    	public void handleMessage(Message msg) {
			...
			case DOWN_ERROR:
				downloadDialog.dismiss();
				Toast.makeText(mContext, "无法下载安装文件,去网站更新吧", 3000).show();
			}
    	};
    };
	...
	public void checkAppUpdate(Context context, final boolean isShowMsg){
		...
		this.appContext = (AppContext)((Activity)context).getApplication();
		this.currentCode = appContext.getVersionCode();
//		getCurrentVersion();
		if(isShowMsg){
			if(mProDialog == null)
		...
		final Handler handler = new Handler(){
			public void handleMessage(Message msg) {
				...
				if(msg.what == 1){
					...
					if(mUpdate.isNeedUpdate(currentCode)){
...
//					ParseResult pr = JsonParseUtil.parseCheckUpdate(update);
					AppUpdate update = appContext.getAppUpdate();
					msg.what = 1;
					msg.obj = update;
...
//	}
	...
	private void showNoticeDialog(){
		...
		builder.setIcon(R.drawable.down2);
		...
		builder.setNegativeButton("以后再说", new OnClickListener() {			
			@Override
			public void onClick(DialogInterface dialog, int which) {
				appContext.setHasNewVersion(true);
				dialog.dismiss();				
			}
		});
	...
	private Runnable mdownApkRunnable = new Runnable() {	
		...
		public void run() {
			try {
				String apkName = "AccountApp_"+mUpdate.getVersionName()+".apk";
				String tmpApk = "AccountApp_"+mUpdate.getVersionName()+".tmp";
				//判断是否挂载了SD卡
				String storageState = Environment.getExternalStorageState();		
				if(storageState.equals(Environment.MEDIA_MOUNTED)){
					savePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/CHAccountant/Update/";
