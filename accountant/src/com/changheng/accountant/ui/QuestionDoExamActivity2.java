 ...
 * 
 */
public class QuestionDoExamActivity2 extends BaseActivity implements
		OnClickListener {
	// 组件
	private ImageButton answerBtn,favoriteBtn,removeBtn;
	...
	private RelativeLayout ruleTypeLayout;
	...
	private void initView() {
		...
		this.ruleTypeLayout = (RelativeLayout) this
		...
		this.material = (QuestionMaterialLayout) this.findViewById(R.id.questionMaterial);
		...
		// 绑定事件
		this.findViewById(R.id.previousBtn).setOnClickListener(this);
		this.findViewById(R.id.nextBtn).setOnClickListener(this);
		this.findViewById(R.id.exitExamImgBtn).setOnClickListener(this);
	...
	private void initData() {
		...
		new Thread() {
			public void run() {
				...
				if ("DoExam".equals(action)) {
					// 题目数据
					questionList = (ArrayList<ExamQuestion>) dao
							.findQuestionByPaperId(paperid);
					record = dao.insertRecord(
							new ExamRecord(paperid, username), 0);
					...
					record = dao.insertRecord(
							new ExamRecord(paperid, username), 0);
					initQuestionAnswer(record.getAnswers());
				}else if("showNoteSource".equals(action))
				{
					questionList = (ArrayList<ExamQuestion>) dao.findQuestionById(intent.getStringExtra("noteLinkedQid"));
				}else if("myErrors".equals(action))
				{
					questionList =(ArrayList<ExamQuestion>) dao.findQuestionFromErrors(username, intent.getStringExtra("sqlStr"));
				}else if("myFavors".equals(action))
				{
					questionList = (ArrayList<ExamQuestion>) dao.findQuestionFromFavors(username, paperid);
				}
...
//			dao.deleteFavor(favor);
	...
	private void gotoChooseActivity() {
		...
		if ("DoExam".equals(action)) {
			...
			mIntent.putExtra("action", "myErrors");
		...
		lv_group.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> adapterView, View view,
				...
				ExamRule rule = QuestionDoExamActivity2.this.ruleList
						.get(position);
				System.out.println("postion = " + position + " rule = "
						+ rule.getRuleTitle());
				...
				QuestionDoExamActivity2.this.examTypeTextView.setText(rule
						.getRuleTitle());
				...
				QuestionDoExamActivity2.this.viewFlow
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				if ("DoExam".equals(q2.action)
					...
					// if ("showQuestionWithAnswer".equals(q2.action))
					// q2.answerBtn
					// .setImageResource(R.drawable.exam_answer_img);
