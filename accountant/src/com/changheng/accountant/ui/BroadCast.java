...
public class BroadCast extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		{			
			//通知栏显示
			this.notification(context, intent.getStringExtra("title"),intent.getIntExtra("newReplyCount", 0));
		}
	}

	private void notification(Context context,String title,int noticeCount){		
		//创建 NotificationManager
		NotificationManager notificationManager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
		
		String contentTitle = "会计学园";
		String contentText = "您的帖子 "+title+" 有 " + noticeCount + " 条最新回复";
		...
		{
			String noticeTitle = "您的帖子 "+title+" 有 " + (noticeCount-_lastNoticeCount) + " 条新回复";
