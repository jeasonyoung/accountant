...
import com.changheng.accountant.ui.BaseActivity;
import com.changheng.accountant.ui.QuestionDoExamActivity1;
import com.changheng.accountant.ui.QuestionDoExamActivity2;
import com.changheng.accountant.view.CheckBoxGroup;
import com.changheng.accountant.view.OptionLayout;
...
public class QuestionAdapter extends BaseAdapter {
	...
	public QuestionAdapter(Context context, BaseActivity activity,
		...
		this.imageSavePath = Environment.getExternalStorageDirectory()
				.getPath()+ File.separator+ "CHAccountant"+ File.separator+ username+ File.separator+ "image"+ File.separator+ "question";
		...
		noteBtnListener = new NoteBtnListener(activity);
	...
	public View getView(int position, View v, ViewGroup parent) {
		...
		if (v == null) {
			System.out
					.println("QuestionAdapter调用getView方法，View===NULL position = "
							+ position);
			...
			contentHolder.noteBtn = (Button) v.findViewById(R.id.btn_note);
		...
		answerHolder.myAnswerTextView = (TextView) v
				.findViewById(R.id.myAnswerTextView); // 我的答案
		answerHolder.sysAnswerTextView = (TextView) v
				.findViewById(R.id.sysAnswerTextView); // 正确答案
		answerHolder.answerResultImg = (ImageView) v
				.findViewById(R.id.answerResultImg); // 判断图片
		answerHolder.analysisTextView = (TextView) v
		...
		if ("1".equals(type) || "2".equals(type) || "3".equals(type)) { // 单选题
			contentHolder.modeLayout.setVisibility(0);
			String[] arr = currentQuestion.getRealContent().replaceAll("\n", "")
					.replaceAll("&nbsp;", " ").split("###");
			String title = arr[0];
			// 显示图片
		...
		// }
			...
			// 加载图片
			String title = currentQuestion.getRealContent().replaceAll("&nbsp;",
					" ");
			// 显示图片
			String zuheName = currentQuestion.getRuleId() + "-"
					+ currentQuestion.getQid();
			...
			contentHolder.modeLayout4.setVisibility(8);
			String title = currentQuestion.getRealContent();
			// 显示图片
			String zuheName = currentQuestion.getRuleId() + "-"
					+ currentQuestion.getQid();
			;
		...
		if (!"DoExam".equals(action)) {
			if (("practice".equals(action) || "myErrors".equals(action))
...
//				contentHolder.examOption.forbidden(true);
			} else if (("practice".equals(action) || "myErrors".equals(action) || "showQuestionWithAnswer"
					.equals(action)) && currentQuestion.getUserAnswer() != null) {
...
//				contentHolder.examOption.forbidden(false);
	...
	// 显示答案
	public void showAnswer(AnswerViewHolder holder,
			ExamQuestion currentQuestion, String userAnswer) {
	...
	private class CheckBoxClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			System.out.println("点击了选项，选项被点击了");
	...
	//笔记按钮监听
	private class NoteBtnListener implements OnClickListener {
		private BaseActivity activity;

		public NoteBtnListener(BaseActivity activity) {
			// TODO Auto-generated constructor stub
			this.activity = activity;
		}
