...
import com.changheng.accountant.db.ImportDBManager;
...
public class PaperDao2 {
	private ImportDBManager dbManager;
	private static final String TAG = "PaperDao";
	public static final int pagesize = 10;

	public PaperDao2(Context context,int areaCode) {
		dbManager = new ImportDBManager(context,areaCode);
	}
	...
	public List<ExamQuestion> findQuestionsByRuleId(String ruleId) {
		...
		String sql = "select qid,ruleid,knowledgeid,classid,content,answer,analysis,linkqid,qtype,optionnum,orderid,randnum,material from ExamQuestionTab where ruleid = ? order by orderid asc";
	...
	public List<ExamQuestion> findQuestionFromErrors(String username,
		...
		// String answer, String analysis, String linkQid,
		// int qType, int optionNum, int orderId
		String sql = "select q.qid,q.ruleid,q.knowledgeid,q.classid,q.content,q.answer,q.analysis,q.linkqid,q.qtype,q.optionnum,q.orderid,q.randnum,q.material from ExamQuestionTab q join ExamErrorQuestionTab e on q.qid = e.qid where "
...
//		System.out.println("classid=== "+classid);
		String sql = "select q.qid,q.ruleid,q.knowledgeid,q.classid,q.content,q.answer,q.analysis,q.linkqid,q.qtype,q.optionnum,q.orderid,q.randnum,q.material from ExamQuestionTab q,ExamFavorTab f where q.qid = f.qid and q.classid = ? and f.username = ? and f.status = 0 order by q.ruleid asc,q.qid asc";
	...
	public ExamRecord findRecord(String username, String paperId) {
		...
		String[] params = null;
		if (username != null) {
			sql = "select r.paperid,r.username,r.answers,r.tempanswer,r.score,r.usetime,r.temptime,r.lasttime,r.isDone,r.mode,r.errornum,p.papername,p.papertime,p.paperscore from ExamRecordTab r,ExamPaperTab p where r.paperid = p.paperid and r.paperid = ? and r.username = ?";
			params = new String[] { paperId, username };
		} else {
			sql = "select r.paperid,r.username,r.answers,r.tempanswer,r.score,r.usetime,r.temptime,r.lasttime,r.isDone,r.mode,r.errornum,p.papername,p.papertime,p.paperscore from ExamRecordTab r,ExamPaperTab p where r.paperid = p.paperid and r.paperid = ?";
		...
		if (cursor.moveToNext()) {
			...
			r.setPaperscore(cursor.getInt(13));
		}
		cursor.close();
		dbManager.closeDatabase();
		return r;
	...
	public List<ExamRecord> findRecordsByUsername(String username) {
		SQLiteDatabase db = dbManager.openDatabase();
		String sql = "select r.paperid,r.username,r.answers,r.tempanswer,r.score,r.usetime,r.temptime,r.lasttime,r.isDone ,p.papername,p.papertime,p.paperscore,r.mode,r.errornum  from ExamRecordTab r,ExamPaperTab p where r.paperid = p.paperid and username = ?";
...
//						+ StringUtils.toDateStr(c.getTime()));
	...
	public StringBuilder findFavorQids(String username, String paperId) {
		...
		String[] params = null;
		if (paperId != null) {
			sql = "select qid from ExamFavorTab where username = ? and paperId = ? and status = 0";
			...
			if (list != null && list.size() > 0) {
				...
				for (Paper p : list) {
					...
					if (cursor.getCount() > 0) {
						...
						db.execSQL("insert into ExamPaperTab(paperid,papername,addtime,paperscore,papertime,year,clicknum,price,classid,totalnum)values(?,?,?,?,?,?,?,?,?,?)",
								new Object[] { p.getPaperId(),
								p.getPaperName(), p.getAddDate(),
								p.getPaperSorce(), p.getPaperTime(),
								p.getYear(), p.getClickNum(), p.getPrice(),
								p.getClassId(), p.getTotalNum() });
					}
				}
				...
				for (ExamRecord r : recordList) {
					...
					if (cursor.getCount() > 0) {
						...
						cursor.close();
						db.execSQL("insert into ExamRecordTab(paperid,username,score,usetime,temptime,answers,tempanswer,lasttime,isDone,mode,errornum,issync)values(?,?,?,?,?,?,?,?,?,?,?,1)", 
								new Object[] { r.getPaperId(),
								r.getUsername(), r.getScore(), r.getUseTime(),
								r.getTempTime(), r.getAnswers(),
								r.getTempAnswer(), r.getLastTime(),
								r.getIsDone(), r.getMode(), r.getErrorNum() });
					}
				}
			}
	...
	public void insertAddTime()
	{
		SQLiteDatabase db = dbManager.openDatabase();
		db.beginTransaction();
		try {
			db.execSQL("insert into DataAddTimeTab (did) values(1)");
			db.setTransactionSuccessful();
		}finally{
			db.endTransaction();
		}
